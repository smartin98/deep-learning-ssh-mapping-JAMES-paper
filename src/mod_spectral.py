...
def compute_segment_alongtrack_output_day(time_alongtrack, 
                               lat_alongtrack, 
                               lon_alongtrack, 
                               ssh_alongtrack,
                               mdt_alongtrack,
    ...
    list_lon_segment = []
    list_ssh_alongtrack_segment = []
    list_mdt_alongtrack_segment = []
    list_ssh_map_interp_segment = []
    list_time_alongtrack_segment = []
    list_lon_alongtrack_segment = []
    list_lat_alongtrack_segment = []
    ...
    if selected_track_segment.size > 0:
        ...
        for track in selected_track_segment:
            ...
            for sub_segment_point in range(start_point, end_point - npt, int(npt*segment_overlapping)):
                ...
                ssh_alongtrack_segment = np.ma.masked_invalid(ssh_alongtrack[sub_segment_point:sub_segment_point + npt])
                mdt_alongtrack_segment = np.ma.masked_invalid(mdt_alongtrack[sub_segment_point:sub_segment_point + npt])
                time_alongtrack_segment = np.ma.masked_invalid(time_alongtrack[sub_segment_point:sub_segment_point + npt])
                lon_alongtrack_segment = np.ma.masked_invalid(lon_alongtrack[sub_segment_point:sub_segment_point + npt])
                lat_alongtrack_segment = np.ma.masked_invalid(lat_alongtrack[sub_segment_point:sub_segment_point + npt])

                ssh_map_interp_segment = []
                ssh_map_interp_segment = np.ma.masked_invalid(ssh_map_interp[sub_segment_point:sub_segment_point + npt])
                if np.ma.is_masked(ssh_map_interp_segment):
                    ssh_alongtrack_segment = np.ma.compressed(np.ma.masked_where(np.ma.is_masked(ssh_map_interp_segment), ssh_alongtrack_segment))
                    mdt_alongtrack_segment = np.ma.compressed(np.ma.masked_where(np.ma.is_masked(ssh_map_interp_segment), mdt_alongtrack_segment))
                    ssh_map_interp_segment = np.ma.compressed(ssh_map_interp_segment)
                    time_alongtrack_segment = np.ma.compressed(time_alongtrack_segment)
                    lon_alongtrack_segment = np.ma.compressed(lon_alongtrack_segment)
                    lat_alongtrack_segment = np.ma.compressed(lat_alongtrack_segment)

                if ssh_alongtrack_segment.size > 0:
                    list_ssh_alongtrack_segment.append(ssh_alongtrack_segment)
                    list_mdt_alongtrack_segment.append(mdt_alongtrack_segment)
                    list_lon_segment.append(mean_lon_sub_segment)
                    list_lat_segment.append(mean_lat_sub_segment)
                    list_ssh_map_interp_segment.append(ssh_map_interp_segment)
                    list_time_alongtrack_segment.append(time_alongtrack_segment)
                    list_lon_alongtrack_segment.append(lon_alongtrack_segment)
                    list_lat_alongtrack_segment.append(lat_alongtrack_segment)


    return list_lon_segment, list_lat_segment, list_ssh_alongtrack_segment, list_mdt_alongtrack_segment, list_ssh_map_interp_segment, list_time_alongtrack_segment, list_lon_alongtrack_segment, list_lat_alongtrack_segment, npt
