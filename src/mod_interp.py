...
def interp_on_alongtrack(gridded_dataset, 
    ...
    # if 'OSE_ssh_mapping' in gridded_dataset:
    # ssh_alongtrack = (ds_alongtrack["sla_filtered"] + ds_alongtrack["mdt"] - ds_alongtrack["lwe"]).values
    ssh_alongtrack = (ds_alongtrack["sla_unfiltered"] + ds_alongtrack["mdt"]).values
    ssh_alongtrack_filtered = (ds_alongtrack["sla_filtered"] + ds_alongtrack["mdt"]).values
    mdt_alongtrack = ds_alongtrack["mdt"].values
    # ssh_alongtrack = (ds_alongtrack["sla_filtered"] + ds_alongtrack["mdt"]).values

    # else:
    # ssh_alongtrack = ds_alongtrack["sla_filtered"].values
    ...
    ssh_alongtrack_filtered = np.ma.masked_where(msk, ssh_alongtrack_filtered).compressed()
    mdt_alongtrack = np.ma.masked_where(msk, mdt_alongtrack).compressed()
    ...
    return time_alongtrack[indices], lat_alongtrack[indices], lon_alongtrack[indices], ssh_alongtrack[indices],mdt_alongtrack[indices], ssh_map_interp[indices], ssh_alongtrack_filtered[indices]
